document.addEventListener('DOMContentLoaded', () => {
    ...
    function activarTab(nombre) {
        ...
        // Detener puzzle al cambiar de pestaña
        if (nombre !== 'puzzle') {
            detenerFisicaPuzzle();
    ...
    function iniciarPuzzle() {
        ...
        for (let i = 0; i < FILAS * COLUMNAS; i++) {
            ...
            const xInicial =
                Math.random() *
                (window.innerWidth - (tamanoPieza + 20));
    ...
    function handleTouchEnd(e, pieza) {
        ...
        const espacioDestino =
            elementoBajoCursor
                ? elementoBajoCursor.closest('.espacio-puzzle')
                : null;
    ...
    function loopFisica() {
        ...
        piezasFlotantes.forEach(item => {
            ...
            const oscilacionX =
                Math.sin(item.balanceo) * 0.4;

            item.x += oscilacionX;


            const anchoPieza =
                item.element.offsetWidth || 80;
    ...
    function colocarPiezaEnCasilla(
        ...
        // Si ya existe una pieza, devolverla al área flotante
    ...
    const gridMemoria =
        document.getElementById('grid-memoria');

    const txtMovimientos =
        document.getElementById('movimientos-memoria');

    const txtPares =
        document.getElementById('pares-memoria');

    const btnReiniciarMemoria =
        document.getElementById('btn-reiniciar-memoria');
    ...
    const areaReflejos =
        document.getElementById('area-reflejos');

    const txtScore =
        document.getElementById('score-reflejos');

    const txtTiempo =
        document.getElementById('tiempo-reflejos');

    const btnIniciarReflejos =
        document.getElementById('btn-iniciar-reflejos');
    ...
    function resetearReflejos() {
        ...
        // CORREGIDO:
        // Se utilizan backticks para texto multilínea.
        if (areaReflejos) {

            areaReflejos.innerHTML = `
                <p>
                    Haz clic en "Iniciar Juego" para atrapar
                    la mayor cantidad de flores antes de
    ...
    function aparecerFlor() {
        ...
        // La flor desaparece después de 1.2 segundos
        setTimeout(() => {
